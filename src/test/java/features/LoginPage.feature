...
Feature:Login Page
  ...
  Scenario Outline: BC01|| Check that user will see an error message if he leaves username's field blank
  ...
  Scenario Outline: BC02|| Check that user will see an error message if he leaves password's field blank
    Given User navigates to herokuApp LoginPage
  ...
  Scenario: BC03|| Check that user will see an error message if he leaves both username and password fields blank
    Given User navigates to herokuApp LoginPage
    When  User clicks on Login button
    Then  Invalid username error message will appear

  Scenario Outline: BC04|| Check that user will see an error message if he enters invalid username
    Given User navigates to herokuApp LoginPage
  ...
  Scenario Outline: BC05|| Check that user will see an error message if he enters invalid password
    Given User navigates to herokuApp LoginPage
    When  User types valid "<username>" in username textBox
  ...
  Scenario Outline: BC06|| Check that user will see an error message if he enters invalid username and password
    Given User navigates to herokuApp LoginPage
    When  User types invalid "<username>" in username textBox
    And   User types invalid "<password>" in password textBox
    And   User clicks on Login button
  ...
  Scenario Outline: GC07|| Check that user will be redirected to Secure Area when he enters valid credentials
    Given User navigates to herokuApp LoginPage
  ...
  Scenario: GC08|| Check text of elements that are displayed in login Page
    Given User navigates to herokuApp LoginPage
